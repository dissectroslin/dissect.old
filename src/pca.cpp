...
PCA::PCA(Kernel * grm)
{
  ...
  grm->kernel->eigenDecomposition(eigenValues, eigenVectors);
  ...
  {
    if(this->individuals.size() != this->globalEigenVectors.size())
    {
      misc.error("Error: An internal error was happened when computing the pca.", 0);
    }
    
    Message message(options.outFile + ".pca.eigenvalues");
    for(int i = (int(globalEigenValues.size()) - 1); i>=(int(globalEigenValues.size())-options.nEigenValues); i--)
    {
