...
Genotype::Genotype(std::string fname)
{
  this->genotypes = new Matrix(MATRIX_DEFAULT_DISTRIBUTION);
  this->missings = new Matrix(MATRIX_DEFAULT_DISTRIBUTION);
...
void Genotype::readFAMFile(std::string f)
{
  std::ifstream file;
  std::string line;
  ...
  if(communicator->mpiRoot)
  {
    misc.message << "Reading Individuals data from file [ " << f << " ] ..." << std::endl;
    misc.checkFileExists(f);
    file.open(f.c_str());
...
void Genotype::readBEDFile(std::string f)
{
  std::ifstream file;
...
void Genotype::parseSNP(int nrf, int rest, unsigned char * memBlock, double *genotypes, double *missings, SNP* snp)
{
  for(int j=0; j<nrf; j++)
  {
    //parseSNPbyte(memBlock[j], 4, 0, &(genotypes[j*4]), &(missings[j*4]), snp);
    parseSNPbyte(memBlock[j], 4, 0, &(genotypes[j*4*this->genotypes->nBlockRows]), &(missings[j*4*this->genotypes->nBlockRows]), snp);
  }
  parseSNPbyte(memBlock[nrf], rest, 0, &(genotypes[nrf*4*this->genotypes->nBlockRows]), &(missings[nrf*4*this->genotypes->nBlockRows]), snp);
...
void Genotype::parseSNPbyte(unsigned char g, int n, int d, double * genotypes, double * missings, SNP* snp)
{
  ...
  for(unsigned int i=0; i<n; i++)
  {
    ...
    {
      genotypes[i*this->genotypes->nBlockRows] = 1;
      missings[i*this->genotypes->nBlockRows] = 1;
      snp->frequencies[ 1 ]++;
    }
    else if( temp2 == 0x3 ) //homozygot
    {
      genotypes[i*this->genotypes->nBlockRows] = 3;
      missings[i*this->genotypes->nBlockRows] = 1;
      snp->frequencies[ 3 ]++;
    }
    else if( temp2 == 0x2 ) //heterozygot
    {
      genotypes[i*this->genotypes->nBlockRows] = 2;
      missings[i*this->genotypes->nBlockRows] = 1;
      snp->frequencies[ 2 ]++;
    }
    else if( temp2 == 0x1 ) //missing
    {
      genotypes[i*this->genotypes->nBlockRows] = 0;
      missings[i*this->genotypes->nBlockRows] = 0;
      snp->frequencies[ 0 ]++;
    }
    else
    {
...
void Genotype::printGenotype(bool showGroups)
{
  double *g;
