...
SNPEffect PredictPhenotype::loadREMLEffect(std::string f, std::string line)
{
  ...
  {
    misc.error("Error: The SNP " + effect.name + " have not a valid number in column 3 of file [ " + f + " ].", 0);
  }
  if( (sstemp >> effect.stdev).fail() )
  {
    misc.error("Error: The SNP " + effect.name + " have not a valid number in column 4 of file [ " + f + " ].", 0);
  }
  if( (sstemp >> effect.mean).fail() )
  {
    misc.error("Error: The SNP " + effect.name + " have not a valid number in column 5 of file [ " + f + " ].", 0);
  }
  if( (sstemp >> effect.neffect).fail() )
  {
    misc.error("Error: The SNP " + effect.name + " have not a valid number in column 6 of file [ " + f + " ].", 0);
  }
...
SNPEffect PredictPhenotype::loadGWASEffect(std::string f, std::string line)
{
  ...
  if( (sstemp >> effect.neffect).fail() )
  {
    misc.error("Error: The SNP " + effect.name + " have not a valid number in column 7 of file [ " + f + " ].", 0);
  }
...
void PredictPhenotype::storePredictions()
{
  ...
  if(communicator->mpiRoot)
  {
    ...
    for(int i = 0; i < this->individuals.size(); i++)
    {
      ...
      message << " " << this->globalPhenotypes[i];
