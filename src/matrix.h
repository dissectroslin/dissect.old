...
public:
   ...
   * \param t2 'T' or 'N'. 'T' indicates the transpose of matrix m2 is used.
   */
  void multiplyMatrixVector(Matrix * m1, char t1, Matrix * v1, char t2, double scale, subMatrix smr);
   ...
   * 
   * Create an intersection matrix from a generating vector. If the element vector element i is equal to vector element j,
   * then element i,j of the matrix will be onIntersection, background otherwise.
   * 
   * \param categories The vector for generating the intersection matrix. Only needs to be defined on root.
   * \param onIntersection The value put on element ij of the matrix when vector element i equals vector element j
   * \param background value for the matrix background.
   */
  void makeIntersectionMatrix(std::vector<int> categories, double onIntersection = 1., double background = 0.);
   ...
   */
  void fillWithRandom(double min = -10., double max = 10.);
