...
void Matrix::generalResorting(Matrix *resultantMatrix, std::map<int, int> & rowsOriginDestination, std::map<int, int> & colsOriginDestination, bool unallocateThisMatrix)
{
  ...
  {
    misc.error("Error: Resorting an empty matrix.", 0);
  }
  ...
  MPI_Alltoall( &(nSendIndexes[0]), 1, MPI_INT, nReceivedIndexes, 1, MPI_INT, MPI_COMM_WORLD);
  ...
  MPI_Alltoallv( &(sendIndexes[0]), &(nSendIndexes[0]), &(sendIndexesDisplacements[0]), MPI_INT, receivedIndexes, nReceivedIndexes, &(receivedIndexesDisplacements[0]), MPI_INT, MPI_COMM_WORLD);
  ...
  MPI_Alltoallv( sendValues, &(nSendValues[0]), &(sendValuesDisplacements[0]), MPI_DOUBLE, receivedValues, &(nReceivedValues[0]), &(receivedValuesDisplacements[0]), MPI_DOUBLE, MPI_COMM_WORLD);
...
void Matrix::multiply(Matrix * m1, char t1, Matrix * m2, char t2, double scale, subMatrix smr)
{
  ...
  else
  {
    ...
    {
      multiplyMatrixVector(m2, t1, m1, t2, scale, smr); //Needs to be implemented
    }
...
void Matrix::multiplyMatrixVector(Matrix * m1, char t1, Matrix * v1, char t2, double scale, subMatrix smr)
{
  int m = m1->nGlobRows;
  int n = v1->nGlobCols;
  double alpha = scale;
  ...
  multiplyMatrixMatrix(m1, t1, v1, t2, scale, smr);
}
...
{
  if( this->symmetric == false || m1->symmetric == false )
  {
    misc.error("Error: An internal error was happened: Trying to compute the trace of a product of a two non-symmetric matrices.", 0);
  }
  ...
  {
    checkMatrixStructure(m1);

    if( (this->nBlockRows != m1->nBlockRows) || (this->nBlockCols != m1->nBlockCols) || (this->nRows != m1->nRows) || (this->nCols != m1->nCols))
    {
    ...
    for(int c = 0; c<this->nCols; c++)
    {
      for(int r = 0; r<this->nRows; r++)
      {
        sum += this->m[c*this->nRows + r] * m1->m[c*this->nRows + r];
      }
    }
...
{
  if( (this->nGlobRows != m1->nGlobRows) || (this->nGlobCols != m1->nGlobCols) || (this->nRows != m1->nRows) || (this->nCols != m1->nCols))
  {
...
void Matrix::makeIntersectionMatrix(std::vector<int> categories, double onIntersection, double background)
{
  int dimension;
  ...
  for(int c = 0; c < nCols; c++)
  {
    ...
    {
      this->m[c*this->nRows + r] = ((rv[r]==cv[c])?onIntersection:background);
    }
  }
...
void Matrix::fillWithRandom(double min, double max)
{
  //srand(time(NULL));
